...
with st.sidebar:
    ...
    tab1 = st.tabs(["프롬프트"])[0]
    updated_examples = []
    # 각 예시 텍스트 입력에 고유한 키(key) 지정
    for i in range(1, len(examples) + 1):
        q = tab1.text_area(f"예시 프롬프트{i} 질문", value=examples[i-1]["question"], height=200, key=f"q{i}")
        a = tab1.text_area(f"예시 프롬프트{i} 답변", value=examples[i-1]["answer"], height=80, key=f"a{i}")
        updated_examples.append({"question": q, "answer": a})
        
    user_text_apply_btn = tab1.button("프롬프트 적용", key="apply1")
    if user_text_apply_btn:
        tab1.markdown("✅ 프롬프트가 적용되었습니다")
        # updated_examples를 기반으로 FewShotPromptTemplate 생성
